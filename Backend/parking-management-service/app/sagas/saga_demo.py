#!/usr/bin/env python3
"""
SAGA Demo Script - Otopark Yönetim Sistemi için Demo Saga Senaryosu

Bu script, SAGA pattern'in çalışma şeklini göstermek için hazırlanmış bir demo scripttir.
Gerçek servisleri çağırmak yerine mock servisler kullanarak SAGA pattern'in nasıl çalıştığını
gösterir.

Örnek senaryo: Araç giriş işlemi için SAGA pattern uygulaması
"""
...
class SagaStep:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """Step bilgilerini dict olarak döndürür"""
        return {
...
class Saga:
    ...
    def to_dict(self) -> Dict[str, Any]:
        """SAGA bilgilerini dict olarak döndürür"""
        return {
...
# --- Mock Servisler --- #
class MockService:
    """Mock servis temel sınıfı"""
    def __init__(self, name: str, failure_rate: float = 0.2):
        self.name = name
        self.failure_rate = failure_rate  # Başarısızlık oranı (0.0 - 1.0)
    ...
    async def call(self, operation: str, data: Dict[str, Any]) -> Tuple[bool, Dict[str, Any]]:
        ...
        # Rasgele başarısızlık simülasyonu
        ...
        await asyncio.sleep(random.uniform(0.1, 0.5))  # Gerçek servis gecikmesi simülasyonu
    ...
    def _generate_response(self, operation: str, data: Dict[str, Any]) -> Dict[str, Any]:
        """Mock yanıt oluşturur - alt sınıflar tarafından uygulanmalıdır"""
        return {"status": "success", "operation": operation}


class LicensePlateService(MockService):
    """Plaka Tanıma Servisi (Mock)"""
    ...
    def _generate_response(self, operation: str, data: Dict[str, Any]) -> Dict[str, Any]:
        if operation == "verify_license_plate":
            ...
            # Çıkış kaydı oluşturulduğunda ücret hesaplama
            hours = random.randint(1, 5)
            hourly_rate = 20  # TL/saat
...
class SagaOrchestrator:
    """SAGA Orchestrator - İşlemleri koordine eder"""
    def __init__(self):
        self.services = {
            "license_plate_service": LicensePlateService(),
            "user_service": UserService(),
            "parking_mgmt_service": ParkingManagementService(failure_rate=0.4),  # Bu servis daha sorunlu
    ...
    async def _compensate_saga(self, saga: Saga) -> None:
        ...
        for step in reversed(compensation_steps):
            ...
            # Telafi operasyonları - gerçek projede her adımın kendine özel telafi işlemleri olur
...
async def run_successful_saga():
    """Başarılı SAGA senaryosu: Araç Giriş İşlemi"""
    logger.info("=== BAŞARILI SAGA SENARYOSU BAŞLIYOR ===")
    
    orchestrator = SagaOrchestrator()
    
    # Bütün servisleri daha güvenilir yap
...
async def run_failing_saga():
    """Başarısız SAGA senaryosu (telafi işlemleri ile): Araç Giriş İşleminde Hata"""
    logger.info("=== BAŞARISIZ SAGA SENARYOSU BAŞLIYOR ===")
    
    orchestrator = SagaOrchestrator()
    
    # Otopark Yönetim Servisi'ni çok yüksek hata oranı ile ayarla
    orchestrator.services["parking_mgmt_service"].failure_rate = 0.9
    
    # SAGA oluştur
    saga = Saga(str(uuid.uuid4()), "Başarısız Araç Giriş İşlemi")
    
    # SAGA adımlarını tanımla (başarılı senaryo ile aynı)
    saga.add_step(SagaStep("verify_license_plate", "license_plate_service", "verify_license_plate"))
...
async def run_vehicle_exit_saga():
    """Araç Çıkış İşlemi SAGA senaryosu"""
...
# --- Ana Demo Programı --- #
async def main():
    """Demo programı - farklı SAGA senaryolarını çalıştırır"""
    logger.info("SAGA Pattern Demo Başlıyor...")
    
    # Başarılı senaryo - Araç Giriş
    await run_successful_saga()
    
    # Başarısız senaryo - telafi işlemleri
    await run_failing_saga()
    
    # Araç Çıkış senaryosu
